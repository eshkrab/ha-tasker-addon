...
async def async_setup_entry(hass: HomeAssistant, entry: ConfigEntry):
    ...
    async def handle_add_task(call):
        data = call.data
        # Required field: friendly_name (this is your human-friendly name)
        friendly_name = data.get("friendly_name")
        if not friendly_name:
            _LOGGER.error("Task must have a friendly_name")
            return
        description = data.get("description", "")
        start_date = data.get("start_date")  # expecting ISO date string (YYYY-MM-DD)
        ...
        # Optionally, set a state for UI visibility (using task_id as identifier)
        hass.states.async_set(f"{DOMAIN}.{new_task.task_id}", "pending")
    ...
    async def handle_update_task(call):
        ...
        if updated_task:
            ...
            hass.states.async_set(f"{DOMAIN}.{task_id}", "updated")
        else:
    ...
    async def handle_mark_task_done(call):
        ...
        if task:
            ...
            hass.states.async_set(f"{DOMAIN}.{task_id}", "done")
        else:
            _LOGGER.error("No task with id %s found", task_id)
